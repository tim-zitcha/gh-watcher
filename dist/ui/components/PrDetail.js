...
export function PrDetail({ state }) {
        ...
        if (fileCount === 0) {
            ...
            if (currentFile) {
                for (const line of currentFile.lines) {
                    switch (line.type) {
                        case "add":
                            lines.push({ text: line.text, color: "green" });
                            break;
                        case "del":
                            lines.push({ text: line.text, color: "red" });
                            break;
                        case "hunk":
                            lines.push({ text: line.text, color: "cyan" });
                            break;
                        case "file":
                            lines.push({ text: line.text, bold: true });
                            break;
                        default:
                            lines.push({ text: line.text, dimColor: true });
                            break;
                    }
                }
            }
    ...
    while (visible.length < visibleLines)
        visible.push({ text: "" });
    ...
    return (_jsx(Box, { flexDirection: "column", borderStyle: "single", borderColor: state.focusedPanel === "detail" ? "yellow" : "gray", width: "62%", flexShrink: 0, children: visible.map((line, i) => (_jsxs(Box, { justifyContent: "space-between", children: [_jsx(Text, { bold: line.bold, color: line.color, dimColor: line.dimColor, children: truncate(line.text || " ", panelWidth) }), needsScrollbar && (_jsx(Text, { color: i >= thumbStart && i < thumbStart + thumbSize ? "yellow" : "gray", children: i >= thumbStart && i < thumbStart + thumbSize ? "│" : "·" }))] }, i))) }));
}
