...
export function SecurityList({ state, hasOrgs }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "single", borderColor: "cyan", flexGrow: 1, children: [_jsxs(Text, { bold: true, children: [pad("Severity", fixedCols.severity), " ", pad("Repo", fixedCols.repo), " ", pad("Package", fixedCols.pkg), " ", pad("Ecosystem", fixedCols.ecosystem), " ", pad("CVE", fixedCols.cve), " ", pad("Opened", fixedCols.opened), " ", pad("Summary", summaryWidth)] }), isLoading
                ? _jsx(Text, { dimColor: true, children: "Loading\u2026" })
                : _jsxs(Text, { dimColor: true, children: ["Showing ", scrollOffset + 1, "-", scrollOffset + visible.length, " of ", alerts.length, " \u00B7 sort: ", securitySortMode] }), isLoading && _jsx(Text, { dimColor: true, children: "  Fetching security alerts\u2026" }), !isLoading && alerts.length === 0 && _jsx(Text, { dimColor: true, children: "  No open security alerts." }), visible.map((alert, i) => {
                const selected = scrollOffset + i === selectedRowIndex;
                return (_jsxs(Text, { inverse: selected, children: [_jsx(Text, { color: severityColor(alert.severity), children: pad(alert.severity.toUpperCase(), fixedCols.severity) }), " ", pad(alert.repository, fixedCols.repo), " ", pad(alert.package, fixedCols.pkg), " ", pad(alert.ecosystem ?? "-", fixedCols.ecosystem), " ", pad(alert.cveId ?? "-", fixedCols.cve), " ", pad(formatTimestamp(alert.createdAt), fixedCols.opened), " ", pad(alert.summary ?? "-", summaryWidth)] }, `${alert.repository}-${alert.number}`));
            })] }));
}
